...
<div class="container" >
    ...
    <{if ($data.admin) }>
      ...
      <{html_options name='admin_class_id' options=$data.class_list_c selected=$data.class_id class='form-control' onchange="submit();"}> 班級學生繳費名單(收費人數：<{$data.selected_count}>人，在籍人數：<{$data.students_count}>人)

      </div>
      <div class='row'>
        <span class="alert alert-danger">管理員權限!!!!</span>
    ...
    <{foreach  key=key item=stud   from= $data.students }>
        ...
        <{if ($data.selected[$stud.stud_id].selected) }>  <{* 有開列 *}>
          <div class='col-md-2 col-sm-4 col-xs-6'><label>■(<{$stud.class_sit_num}>)<{$stud.name}></label></div>
        <{else}>
          <div class='col-md-2 col-sm-4 col-xs-6'><label><input type='checkbox' name='selected_stud[]' value='<{$stud.stud_id}>_<{$stud.class_sit_num}>' id='stud_selected'>(<{$stud.class_sit_num}>)<{$stud.name}></label></div>
        <{/if}>
    ...
    <div class="form-actions">
        <p>基本上全班學生都開列，除非該學生在這次的轉帳的各種項目費用都無關才不需開列。</p>
         <span class='col-md-2 col-xs-2'><label><input type='checkbox' name='CheckAll'  id='CheckAll'>全選</label> </span>
