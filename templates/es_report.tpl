...
<{if ($data.seletc_item ) }>
	...
	<table class="table table-bordered" >
 	<tr>
      <td class='col-md-2 col-sm-2 col-xs-2'>姓名</td>
  ...
  <{foreach  key=key item=stud   from= $data.students }>
    ...
    <{/if}>
      ...
      <td ><span class="in_bank" id='icon_<{$stud.stud_id}>_<{$data.seletc_item}>' data='1'><span class=" glyphicon glyphicon-shopping-cart" title="銀行扣款"></span></span>(<{$stud.class_sit_num}>)<{$stud.name}></td>
      <{else}>
      <td ><span class="in_bank"  id='icon_<{$stud.stud_id}>_<{$data.seletc_item}>' data='0'>
          <span class='label label-danger'>
              <span class=" glyphicon glyphicon-user" title="自行繳費"></span></span></span>
              (<{$stud.class_sit_num}>)<{$stud.name}></td>
      <{/if}>
       		<!--            繳費金額(有申請減免                   -->
      <{foreach  key=detail_key item=detail_val   from= $data.detail_list }>
         <{if ($data.decase_list_array[$stud.stud_id].dollar[$detail_key]) }>
         <td ><span class="label label-primary"><{$data.pay_list[$stud.stud_id][$detail_key]}></span></td>
         <{else}>
...
<script>
  ...
  $(document).on("click", ".in_bank", function(){
    	...
    	if (in_bank=='1') {
    	 	...
    	 	$('#' + div_id).html( '<span class="label label-danger"><span class=" glyphicon glyphicon-user" title="自行繳費"></span></span>') ;
    	 	$('#' + div_id).attr("data","0") ;
    	}else {
    		$('#' + div_id).html( '<span class=" glyphicon glyphicon-shopping-cart" title="銀行扣款"></span>') ;
    		$('#' + div_id).attr("data", "1") ;
    	}
