...
<{if ($data.inTime)}>
  ...
  <tr>
      <th class='col-md-1 col-sm-1 col-xs-1'>姓名</th>
      <th class='col-md-1 col-sm-1 col-xs-1'>主身份別</th>
      <{foreach  key=detail_key item=detail_val   from= $data.detail_list }>
       <{*       如年段的該項目無收費 則不出現                       *}>
       <{if ($data.detail_dollar.pay[$detail_key])<>0 }>
        <th  class='col-md-1 col-sm-1 col-xs-2'><{$detail_val}>(<{$data.detail_dollar.pay[$detail_key]}>元)
            <div class="alert alert-danger">
            <span class="glyphicon glyphicon-record"    set="detailid_<{$detail_key}>"  dollar="<{$data.detail_dollar.pay[$detail_key]}>"  title='全班先減免此項' >全班扣</span>
        </div>
        </th>
       <{/if}>
      <{/foreach}>
       <th class='col-md-1 col-sm-1 col-xs-2'>備註</th>
    </tr>
      ...
      <{foreach  key=detail_key item=detail_val   from= $data.detail_list }>
       ...
       <div class="row money_div"  id='box_<{$stud.stud_id}>_<{$detail_key}>'  >
          ...
          <span class="col-md-2 col-sm-2 col-xs-2">
            <span class="glyphicon glyphicon-forward"    set="dollars_<{$stud.stud_id}>_<{$detail_key}>"  dollar="<{$data.detail_dollar.pay[$detail_key]}>"  title='(<{$stud.class_sit_num}>)<{$stud.name}> , <{$detail_val}>(<{$data.detail_dollar.pay[$detail_key]}>元)'  ></span>
          </span>
          <{*   -----  金額  ------------------     *}>
          <span class="col-md-9 col-sm-9 col-xs-9">
            <input   class='form-control money detailid_<{$detail_key}>'  name='dollars[<{$stud.stud_id}>_<{$detail_key}>]' type='text' id='dollars_<{$stud.stud_id}>_<{$detail_key}>'  sit_num = '<{$stud.class_sit_num}>'   title='(<{$stud.class_sit_num}>)<{$stud.name}> , <{$detail_val}>(<{$data.detail_dollar.pay[$detail_key]}>元)'  value='<{$data.decase_list[$stud_id].dollar[$detail_key] }>'   onchange="check_input($(this),<{$data.detail_dollar.pay[$detail_key]}>);" />
                      ...
                      <{ if !($data.dent_support[$detail_key]) }>
                        <span  class="col-md-10 col-sm-10 col-xs-10 user_<{$stud_id}>" id = "need_<{$stud.stud_id}>_<{$detail_key}>"
                          <{if ($data.selected[$stud_id].cause_id ==0) }>
                          style="display:none"
                           <{/if}>
                          >
                        <div class='row'>
                        <span class='col-md-2 col-sm-3 col-xs-4'>
                        ...
                        <span class='col-md-2 col-sm-3 col-xs-4'>
                           ...
                           <span class="glyphicon glyphicon-filter" id="showOther_<{$stud.stud_id}>_<{$detail_key}>"  style="background-color: green"     title="第二種身份--<{$data.decase_list[$stud_id].other_cause_str[$detail_key]}> ">
                           <{else}>
                            <span class="glyphicon glyphicon-filter" id="showOther_<{$stud.stud_id}>_<{$detail_key}>"  title="設定第二種身份">
                           <{/if}>
                         </span>
                         </span>
                       </div>
