...
<h3><{$data.class_list_c[$data.class_id]}>  減免學生金額列表(收費人數：<{$data.selected_count}>人，在籍人數：<{$data.students_count}>人)</h3>
    ...
    <tr>
      <div class="row" >
      <td class='col-md-1 col-sm-1 col-xs-1'>座號</td>
      <td class='col-md-1 col-sm-1 col-xs-1'>姓名</td>

      <{foreach  key=detail_key item=detail_val   from= $data.detail_list }>
        <td class='col-md-1 col-sm-1 col-xs-1'><{$detail_val|truncate:9}><span class="label"><{$data.detail_dollar.pay[$detail_key]}></span></td>
      <{/foreach}>

      <td class='col-md-2 col-sm-2 col-xs-2'>減免原因</td>
    ...
    <tr>
      ...
      <td><div class='col-md-1 col-sm-1 col-xs-1'  id="sit_<{$data.seletc_item}>_<{$key}>" data='<{$data.students[$key].class_sit_num}>'  ><{$data.students[$key].class_sit_num}></div></td>
      <td class='col-md-1 col-sm-1 col-xs-1' id="name_<{$data.seletc_item}>_<{$key}>" data='<{$data.students[$key].name}>' ><{$data.students[$key].name}></td>
      ...
      <{foreach  key=detail_key item=detail_val   from= $data.detail_list }>
        ...
        <{if ($data.decase_list[$key].cause_chk[$detail_key])}>
          ...
          <{if ($data.decase_list[$key].other[$detail_key])}>
             <{*   other cause_chk   *}>
            <td class='col-md-1 col-sm-1 col-xs-1' id='dollar_<{$data.seletc_item}>_<{$key}>_<{$detail_key}>' data='<{$data.decase_list[$key].dollar[$detail_key]}>' need='checked'  ><span class="label label-warning" title='申請補助,其它身份別(<{$data.decase_list[$key].other_cause_str[$detail_key]}>)'><{$data.decase_list[$key].dollar[$detail_key]}></span></td>
          <{else}>
            <td class='col-md-1 col-sm-1 col-xs-1' id='dollar_<{$data.seletc_item}>_<{$key}>_<{$detail_key}>' data='<{$data.decase_list[$key].dollar[$detail_key]}>' need='checked'  ><span class="label label-info" title='申請補助'><{$data.decase_list[$key].dollar[$detail_key]}></span></td>
          <{/if}>

        <{else}>
        <td class='col-md-1 col-sm-1 col-xs-1' id='dollar_<{$data.seletc_item}>_<{$key}>_<{$detail_key}>' data='<{$data.decase_list[$key].dollar[$detail_key]}>' need=''  ><{$data.decase_list[$key].dollar[$detail_key]}></td>
        <{/if}>
      <{/foreach}>

      <td class='col-md-2 col-sm-2 col-xs-2' id='cause_<{$data.seletc_item}>_<{$key}>' data='<{$data.selected[$key].cause|truncate:18}>'><span class="del"><i class="glyphicon glyphicon-trash"></i></span><{$data.selected[$key].cause|truncate:18}>
          <{if ($data.selected[$key].ps) }>
            <span class="glyphicon glyphicon-info-sign" title="連絡內容：<{$data.selected[$key].ps}>"></span>
          <{/if}>
        </td>
    ...
    <{/if}>
      ...
      <{if ($data.detail_dollar.pay[$key])<>0 }>
 			<tr>
        <td>
 			<span class="fill" id= "fill_dollars<{$key}>_<{$data.detail_dollar.pay[$key]}>"   title="點選填入金額或清空">
 			<label   for="dollars[<{$key}>]" ><{$detail}>(<{$data.detail_dollar.pay[$key]}>元):</label>
  			...
  			<input    name='dollars[<{$key}>]' type='text' id='dollars<{$key}>'  title='<{$detail}><{$data.detail_dollar.pay[$key]}>'  tabindex='<{$key}>'
  			    placeholder='<{$data.detail_dollar.decease[$key]}>' class="form-control"  onchange="check_input($(this),<{$data.detail_dollar.pay[$key]}>);" />
  			...
  			<div class="input-append">
                      ...
                      <{ if !($data.dent_support[$key]) }>
                        <span class="col-md-4 col-xs-4">
                        <span class="glyphicon glyphicon-filter show_other" title='其他身份別選擇' data_ref='other_<{$key}>'  ></span>
                        </span>
                      <{/if}>
                      </div>
                    <div class="input-append">
   ...
   <{foreach  key=detail_key item=detail_val   from= $data.detail_list }>
    ...
    <{/foreach}>
        ...
        var form_str =
        		...
        		"<div class='col-md-1 col-sm-1 col-xs-1'>"+sit_str+"</div>"+
      			"<div class='col-md-1 col-sm-1 col-xs-1'>"+name_str+"</div>" +

      			<{foreach  key=detail_key item=detail_val   from= $data.detail_list }>
        		"<div class='col-md-1 col-sm-1 col-xs-1'> "+
                       "<input class='col-md-1 col-sm-1 col-xs-10' name='dollars[<{$detail_key}>]' type='text' id='dollars[<{$detail_key}>]'  title='<{$detail_val}>'  tabindex='9<{$detail_key}>' value='"+ dollar_<{$detail_key}> +"'  onblur='check_input($(this),<{$data.detail_dollar.pay[$detail_key]}>);' />" +
