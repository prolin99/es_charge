    ...
    <tr>
      ...
      <{foreach  key=detail_key item=detail_val   from= $data.detail_list }>
        ...
        <{if ($data.decase_list[$key].cause_chk[$detail_key])}>
          ...
          <{else}>
            <td   id='dollar_<{$data.seletc_item}>_<{$key}>_<{$detail_key}>' data='<{$data.decase_list[$key].dollar[$detail_key]}>' need='checked'  ><span class="badge badge-info" title='申請補助'><{$data.decase_list[$key].dollar[$detail_key]}></span></td>
          <{/if}>
