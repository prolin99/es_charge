...
if ($_POST['act_save'] )  {
	...
	$sql = " INSERT INTO " . $xoopsDB->prefix("charge_item") .  
		" (`item_id`,  `item_type`, `item`, `start_date`, `end_date`, `comment`, `creater` )  " .
		"VALUES ('','{$_POST['item_type']}'     ,'{$_POST['item']}'   , '{$_POST['start_date']}'  , '{$_POST['end_date']}'   , '{$_POST['comment']}'  , '$creater'   )" ;
...
if ($_POST['act_edit'] )  {
	$sql = " UPDATE  "  . $xoopsDB->prefix("charge_item") .  
	     	   "  SET item_type = '{$_POST['item_type']}'  ,  `item`='{$_POST['item']}'  ,   `start_date`='{$_POST['start_date']}' , `end_date` ='{$_POST['end_date']}', `comment`='{$_POST['comment']}'  " .
